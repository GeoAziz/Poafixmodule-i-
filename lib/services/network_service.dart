...
class NetworkService {
    ...
    for (final endpoint in endpoints) {
      try {
        ...
        if (response.statusCode != 404) {
          if (kDebugMode) {
            print('✅ Auth endpoint $endpoint exists (status: ${response.statusCode})');
          }
          return true;
        }
      } catch (e) {
  ...
  Future<http.Response> makeRequest(
    ...
    if (_workingBaseUrl == null) {
      throw Exception('No backend server available. Please check your network connection.');
    }
    ...
    try {
      ...
      switch (method.toUpperCase()) {
        case 'GET':
          response = await http.get(url, headers: requestHeaders).timeout(Duration(seconds: 30));
          break;
        case 'POST':
        ...
        case 'PUT':
          ...
          break;
        case 'DELETE':
          response = await http.delete(url, headers: requestHeaders).timeout(Duration(seconds: 30));
          break;
        default:
      ...
      if (_workingBaseUrl != null) {
        ...
        if (_workingBaseUrl != null) {
          return makeRequest(endpoint, method: method, headers: headers, body: body);
        }
      }
